...
class RedshiftSummaryOperator(PythonOperator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        main_sql += "DROP TABLE IF EXISTS {schema}.temp_{table};".format(schema=self.schema, table=self.table)
        ...
        main_sql += (
            "CREATE TABLE {schema}.temp_{table} {attributes} AS ".format(
                schema=self.schema, table=self.table, attributes=attributes
            )
            + self.main_sql
        )

        if after_sql:
            self.after_sql = after_sql.format(schema=self.schema, table=self.table)
        else:
            self.after_sql = ""

        super(RedshiftSummaryOperator, self).__init__(
            python_callable=redshift_sql_function,
            params={"sql": main_sql, "overwrite": overwrite, "redshift_conn_id": self.redshift_conn_id},
    ...
    def execute(self, context):
        ...
        for item in self.input_check:
            ...
            if cnt < item["count"]:
                raise AirflowException("Input Validation Failed for " + str(item["sql"]))
            ...
            if item.get("op") == "eq":
                if int(cnt) != int(item["count"]):
                    raise AirflowException(
                        "Output Validation of 'eq' Failed for "
                        + str(item["sql"])
                        + ": "
                        + str(cnt)
                        + " vs. "
